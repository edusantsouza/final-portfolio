...
export const FormContact = () => {
  ...
  if (msgStatus) {
    return (
      <div className="contact__form-container primary-form">
        ...
        <form ref={form} onSubmit={sendEmail} className='contact__form' id="contact-form">
          <div className="form-item">
            ...
            <input type="text" name='username' value={value.username} onChange={handleInputChanges} placeholder='Insira seu nome' required />
          </div>
          <div className="form-item">
            <label htmlFor="email">E-mail</label>
            <input type="email" name='useremail' value={value.useremail} onChange={handleInputChanges} placeholder='Insira seu e-mail' required />
          </div>
          <div className="form-item">
            <label htmlFor="message">Projeto</label>
            <textarea name="message" value={value.message} onChange={handleInputChanges} id="message" cols="30" rows="10" required ></textarea>
          </div>
    ...
    return (
      <div className="contact__form-container">
        <div className="contact__sucess-message">
          <h2>Mensagem enviada!</h2>
          <i class='bx bx-check-circle check-icon' ></i>
          <button className="button button--flex" onClick={() => {
            setMsgStatus(true)
            setValue({
              username: '',
              useremail: '',
